...
class SpotLight {
	...
	constructor(from, to, intensity, exponent, cutoff) {
		...
		this.to = to.clone();
		this.direction = this.to.clone().sub(this.from).normalize() // points from source to target
		this.intensity = intensity.clone();
		this.exponent = exponent;
		this.cutoff = cutoff 
		cutoff *= 2 * Math.PI / 360 // convert to radians
	}
	getLight(shadingPoint) {
// ===YOUR CODE STARTS HERE===
		// if angle is not within cutoff, return null
		const shadingVector = this.from.clone().sub(shadingPoint).normalize();
		const dot = this.direction.dot(shadingVector)
		if (Math.acos(dot) > this.cutoff) {
			return null
		}

		const sample = (new PointLight(this.from, this.intensity)).getLight(shadingPoint)
		sample.intensity.multiplyScalar(Math.pow(dot, this.exponent))
		return sample
// ---YOUR CODE ENDS HERE---
	}
}
